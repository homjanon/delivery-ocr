...
> ⚡ **SSE 流式 + 超时降级（2026-08-08）**：识别改为流式输出（内容边生成边在「③ 识别结果预览」上方滚动显示，体感提速）；**空闲超时 40s**（流式 40s 内零反馈、无任何思考/输出即中断并自动切换下一个有 Key 的模型；一旦有输出就重置计时、持续输出不打断）；模型偶发输出 JSON 对象而非数组时会自动归一化为数组。
