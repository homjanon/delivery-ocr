...
### 模型预设（网页版）
页面下拉直接切换；**硅基流动浏览器直连（原生 CORS）；Agnes 默认经自有 Cloudflare 代理 `proxy.hellohopo.dpdns.org` 加速**：
...
> 默认预设为 **Agnes 2.0-Flash**（免费无限期，高峰偶有排队）；**硅基流动 Qwen3.5-35B-A3B** 为付费备用。两个模型均关闭思考模式（`enable_thinking:false`）并设 `max_tokens:8192`，避免 JSON 被截断。
> ⚡ **SSE 流式 + 超时降级（2026-08-08）**：识别改为流式输出（内容边生成边在「③ 识别结果预览」上方滚动显示，体感提速）；**空闲超时**（流式 120s 无数据才中断，持续有输出不打断），主通道失败自动切换下一个有 Key 的预设，末位带代理的通道再补一次直连兜底；模型偶发输出 JSON 对象而非数组时会自动归一化为数组。
> 💡 **报错定位**：每个通道失败都会打印「✗ 通道名 失败：原因」；**401 = Key 无效/已注销**（需到平台重新生成并填入），**403 = 无权限/账户余额不足**（Qwen3.5-35B-A3B 为付费模型，需账户有余额）。
> 历史上曾内置「硅基流动 Qwen3.5-397B-A17B」「商汤 SenseNova」「智谱 GLM-4.6v」预设，已移除。
